...
unsigned int nModifierInterval = 11 * 60; // 11 minutes (time to elapse before new modifier is computed)

int nCoinbaseMaturity = 110; // 110 blocks for coins to mature
...
const string strMessageMagic = "bluesteel Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 50) // anti-insta-premine / testing blocks
    {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
   ...
   if (nHeight > 1500 && nHeight < 10000) {

       if (nHeight < 2500)
           nSubsidy = 100 * COIN;

       else if (nHeight < 2750)
           nSubsidy = 250 * COIN;
           ...
           nSubsidy = 100 * COIN;
            ...
            nSubsidy = nCoinAge * (2 *nRewardCoinYear) / 365 / COIN; // 10% instead of 5% for this period
           ...
           nSubsidy = nCoinAge * nRewardCoinYear / 365 / COIN; // reg. 5%

       else if (nHeight < 8800)
           nSubsidy = 50 * COIN;

       else if (nHeight < 9100)
           nSubsidy = 150 * COIN;

       else if (nHeight < 9300)
           nSubsidy = 450 * COIN;
...
static const int64_t nTargetTimespan = 18 * 30;  // 9 mins
//
...
// bluesteel: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "bluesteel", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "I’m pretty sure there’s a lot more to life than being really, really good looking. And I plan on finding out what that is. - Derek. |[quote/timestamp: unrelated]| 6.20.2015";
        CTransaction txNew;
        txNew.nTime = 1434790115;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xd1fb595d6ea6177ac0ab35bde734572055ddb376bb9bef30509d5dcf9a3ff9f4"));
        block.print();
